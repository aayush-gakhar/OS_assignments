...
void readcsv(char section, double arr[]){
    ...
    for (int i = 0; i < 6; i++){
        if(isnan(avg[i]/n))printf("  00.000000  ");
        else printf("  %f  ",avg[i]/n);
        arr[i]+=avg[i];
    }
    arr[6]+=n;
    printf("\n\n");
...
int main(int argc, char const *argv[]){
    ...
    printf("Assignment 1|Assignment 2|Assignment 3|Assignment 4|Assignment 5|Assignment 6\n");
    for (int i = 0; i < 6; i++){
        if(isnan(arr[i]/arr[6]))printf("  00.000000  ");
        else printf("  %f  ",arr[i]/arr[6]);
    }
    printf("\n");
    return 0;
}